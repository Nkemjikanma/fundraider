...
export default function HomePage() {
	...
	useEffect(() => {
		const fetchBalance = async () => {
			try {
				...
				const balanceResponse = await getWalletBalance(fundraisers[0].fundraiserAddress.address);

				setRaised(balanceResponse.balance);

				// Total of all fundraisers on Fundraider
				const fetchTotalFundraids = fundraisers.map(
					async (fundraiser) => await getWalletBalance(fundraiser.fundraiserAddress.address),
				);

				const allFundraids = await Promise.all(fetchTotalFundraids);

				const totalFundraids = allFundraids.reduce((acc, { balance }) => acc + Number(balance), 0);
	...
	const addFrame = useCallback(async () => {
		...
		try {
			...
			if (
				...
				e !== null &&
				"reason" in e &&
				(e.reason === "rejected_by_user" || e.reason === "invalid_domain_manifest")
			) {
	...
	const sendWelcomeNotification = async (notificationDetails: FrameNotificationDetails) => {};
			...
			<SplashContainer>
				...
				<div className="relative w-full space-y-4">
					{fundraisers.map((fundraiser) => (
						<Link key={fundraiser.id} href={"/rosalie"} className="group hover:shadow-md">
